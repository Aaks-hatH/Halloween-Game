// game.js — complete, drop-in (uses WS backend and the admin password you provided)
(function(){
  'use strict';
  ...
  // Admin password stored base64 (your password: Purple&OrangeMouse^2)
  ...
  function checkRiddle(id){
    ...
    if (!r) return;
    const expected = (r.answer || '').toUpperCase();
    if (first === expected) {
      solved[id] = true;
  ...
  function getFinalCode(){ return riddles.map(r => (r.answer || '')[0]?.toUpperCase() || '?').join(''); }
  ...
  function attachUnlock(){
    ...
    btn.addEventListener('click', ()=>{
      ...
      let entered = sanitizeInput(input.value || '').trim().toUpperCase();
  ...
  /* Hints */
  function attachHintButton(){
    const hb = document.getElementById('hintBox');
    const btn = document.getElementById('hintBtn');
    if (!btn || !hb) return;
    btn.addEventListener('click', ()=>{
      if (hintsRemaining <= 0) { hb.style.display='block'; hb.innerHTML = `<p class="no-select" style="font-family:Creepster, cursive; font-size:1.2rem;">⛔ No hints remaining! (${hintsUsed}/${totalHints} used)</p>`; return; }
      const rn = prompt('Which riddle do you want a hint for? (1-5)');
      if (!rn) { hb.style.display='none'; hb.innerHTML=''; return; }
      const n = parseInt(sanitizeInput(rn));
      if (!n || n < 1 || n > 5 || !hints[n]) { hb.style.display='block'; hb.innerHTML = `<p class="no-select" style="font-family:Creepster, cursive; font-size:1.2rem;">That riddle doesn't exist — try 1 to 5.</p>`; return; }
      hb.style.display='block';
      hb.innerHTML = `<p class="no-select" style="font-family:Creepster, cursive; font-size:1.2rem;">💡 Hint for Riddle ${n}: ${hints[n]}<br><br>Hints remaining: ${hintsRemaining - 1}/${totalHints}</p>`;
...
function getAnalytics(){
  ...
  function getAnalytics(){
    try {
      ...
      const bestTime = completeTimes.length ? Math.min(...completeTimes) : 0;
      return { attempts, completions, locked, hints, avgTime, bestTime };
    } catch(e){ return { attempts:0, completions:0, locked:0, hints:0, avgTime:0, bestTime:0 }; }
  }
  ...
  function attachUnlock(){ /* defined above, but ensure present */ }
  ...
  attachUnlock = attachUnlock; // no-op to satisfy linter
  attachHintButton = attachHintButton;
...
// Manual hint function - exposed globally
